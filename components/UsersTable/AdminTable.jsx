...
export default function AdminTable() {
  ...
  const fetchData = async (pageNo = 1, pageSize = 20) => {
    try {
      const response = await fetch(
        `${config.host}/tenant/admin/v2/account?pageNo=${pageNo}&pageSize=${pageSize}`,
        {
          method: "GET",
          headers: {
      ...
      if (response.ok) {
        ...
        const totalItems = data.serviceResponse.adminAccountList.length; // Assuming this is the total number of items
        // const totalPages = Math.ceil(totalItems / pageSize);
        const totalPages = 5;
        setTotalPages(totalPages);
        const filteredData = data.serviceResponse.adminAccountList.filter(
  ...
  const handleAdd = async (newRow) => {
    try {
      const response = await fetch(`${config.host}/tenant/admin/v2/account`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${accessToken}`,
          "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        // Adjust the expected error message based on the actual API response
        const expectedErrorMessage = `Permission denied to create a linked account for admin, can not create multiple login for admin uuidub_837d8a3e-9a08-4ad9-935d-ad7f1a614884 email : ${newRow.email}`;
        ...
        if (normalizedErrorMessage === expectedErrorMessage) {
          ...
          setErrorMessage(
            "Account already exists. Please use different Phone Number or Mail id."
          );

          // Return the error message to be handled by the parent component
          return normalizedErrorMessage;
        } else {
          console.error("Failed to add admin account:", response.status);
          ...
          return "Failed to add admin account. Please try again.";
        }
      }
    } catch (error) {
      ...
      // Set the error message in state
      setErrorMessage("Error adding admin account. Please try again.");

      // Return the error message to be handled by the parent component
      return "Error adding admin account. Please try again.";
    }
  };

  const handleAddUpdate = async (enable, active, name, email, phone, type) => {
    try {
      const newRow = {
        name,
        email,
